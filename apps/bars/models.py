...
def upload_to(instance, filename):
    return "bars/{bar_name}/logo/{filename}".format(
        bar_name=instance.name, filename=filename
    )
...
class Bar(models.Model):
    ...
    name = models.CharField(
        max_length=60, default="Anonymous Bar", blank=False, null=False
    )
...
def upload_to_bar_images_folder(instance, filename):
    return "bars/{bar_name}/images/{filename}".format(
        bar_name=instance.bar, filename=filename
    )
...
class BarImage(models.Model):
    ...
    image = models.ImageField(
        upload_to=upload_to_bar_images_folder, blank=True, null=True
    )
